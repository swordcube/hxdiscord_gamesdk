...
HL_PRIM vresult HL_NAME(runCallbacks)() {
	if (core != nullptr) return (vresult)core->RunCallbacks();
}

HL_PRIM vresult HL_NAME(registerCommand)(vbyte* command) {
	if (core != nullptr) return (vresult)core->ActivityManager().RegisterCommand((const char*)command);
}

HL_PRIM vresult HL_NAME(registerSteam)(uint32_t steamId) {
	if (core != nullptr) return (vresult)core->ActivityManager().RegisterSteam(steamId);
}
